...
STEP_INTERVAL = int(os.getenv("STEP_INTERVAL", "10"))  # seconds between steps
MAX_STEPS = int(os.getenv("MAX_STEPS", "100"))
SLO_LATENCY = float(os.getenv("SLO_LATENCY", "200.0"))  # ms
...
class CanaryController:
    def __init__(self):
        ...
        # Kubernetes client
        try:
            # Load in-cluster config khi chạy trong pod
            config.load_incluster_config()
        except:
            # Load local config khi chạy local
    ...
    def get_observation(self):
        ...
        if canary_metrics is None:
            # Default values nếu không lấy được metrics
            error_rate = 0.0
            latency = 100.0
        else:
    ...
    def _update_ingress_weights(self):
        ...
        Sử dụng NGINX Ingress Controller canary annotations.
        """
        ...
        try:
            # Patch canary ingress với weight mới
            body = {
                "metadata": {
                    "annotations": {
    ...
    def run(self):
        ...
        while self.step_count < self.max_steps:
            ...
            if error_rate > 0.04:  # Critical error
